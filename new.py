...
data = data[data['Stage 2 temp'] <= 400]
# Filter out data where 'Stage 2 set temp' is less than or equal to 300
data = data[data['Stage 2 set temp'] > 300]

# Extract the relevant columns as numpy arrays
time = data['Time'].values
stage1_temp = data['Stage 1 temp'].values
stage2_temp = data['Stage 2 temp'].values
heater1_power = data['Heater 1 power'].values / 1000  # Scale down by 1000
heater2_power = data['Heater 2 power'].values / 1000  # Scale down by 1000
heater3_power = data['Heater 3 power'].values / 1000  # Scale down by 1000
RPM_condition = data['CD Stepper 1 speed'].values

# Calculate the difference between Stage 1 and Stage 2 temperatures
...
min_length = min(len(time_seconds), len(rmsSignal), len(stage2_temp), len(stage1_temp), len(RPM_condition), len(heater1_power), len(heater2_power), len(heater3_power))
...
heater3_power = heater3_power[:min_length]
...
valid_indices = np.where((heater1_power > 0) & (heater2_power > 0) & (heater3_power > 0))[0]
...
num_intersections_Stage2 = count_intersections(stability_grad_diffStage2[start_idx:], specLineStage2)
num_intersections_Stage1 = count_intersections(stability_grad_diffStage1[start_idx:], specLineStage1)
...
plt.plot(time[:min_length], heater3_power[:min_length], label='Heater 3 Power / 1000', color='green', linestyle='--')
...
# Print intersection points and apex point
print(f"Apex point (Time, Temperature): {time[apex_index]}: {stage2_temp[apex_index]}°C")

# Print the average cooldown gradients
print(f"Average cooldown gradient for the 2nd stage: {avg_cooldown_gradient_2nd_stage} K/s")
print(f"Average cooldown gradient for the 1st stage: {avg_cooldown_gradient_1st_stage} K/s")

print(f"1st stage ultimate temperature: {UltimateTempStage1}")
print(f"2nd stage ultimate temperature: {UltimateTempStage2}")

# print(f"1st stage time to ultimate: {TimeUltimateTempStage1}")
# print(f"2nd stage time to ultimate: {TimeUltimateTempStage2}")

print(f"Number of intersections (Stage 1) at {specLineStage1} K/min: {num_intersections_Stage1}")
print(f"Number of intersections (Stage 2) at {specLineStage2} K/min: {num_intersections_Stage2}")

print(f"1st stage 3 SD: {3 * std_stage1}")
print(f"2nd stage 3 SD: {3 * std_stage2}")
